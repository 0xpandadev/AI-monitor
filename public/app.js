...
function renderDigest(){
  ...
  const keyMessages=b.key_messages||[];
  return `<section class="canvas-hero"><div class="canvas-hero-copy"><span>WEEKLY THESIS · ${esc(period)}</span><h2>${esc(b.headline||'今週のAI市場の論点')}</h2><p>${esc(b.summary||'重要な公開情報を原典リンク付きで整理します。')}</p><div class="canvas-brief-messages count-${Math.max(1,keyMessages.length)}">${keyMessages.length?keyMessages.map((message,index)=>`<div><i>${String(index+1).padStart(2,'0')}</i><section><b>${esc(message.title)}</b><small>${esc(message.description)}</small></section></div>`).join(''):`<div><b>${esc(b.headline)}</b><small>今週の結論をここに表示します。</small></div>`}</div></div><aside class="canvas-coverage"><span>RESEARCH COVERAGE</span><dl><div><dt>監視企業</dt><dd>${d.metrics.watched}</dd></div><div><dt>掲載記事</dt><dd>${feed.length+(b.weekly_digest||[]).length}</dd></div><div><dt>基礎情報あり</dt><dd>${d.metrics.profiles_started}</dd></div></dl><p>${esc(d.profile_coverage.note||'3〜5年ベースラインを整備中です。')}</p><button data-jump="settings">調査範囲・判定ルール →</button></aside></section>
  ${renderWeeklyReport(activeFeed,feed,b)}
  ${renderRankingPulse(d.rankings)}
...
function renderRankingPulse(rankings){
  const catalog=new Map((rankings?.catalog?.sources||[]).map(item=>[item.id,item]));
  const snapshots=new Map((rankings?.sources||[]).map(item=>[item.id,item]));
  const sources=[...catalog.values()].map(source=>({...source,...(snapshots.get(source.id)||{})}));
  return `<section class="ranking-board"><header><div><span>RANKING LENSES</span><h2>モデル・利用動向ランキング</h2><p>性能、利用、人間選好、実装能力を分けて、上位モデルだけを確認します。</p></div><aside><b>${sources.filter(item=>item.status==='confirmed').length}</b><small> / ${sources.length} 指標を更新</small></aside></header><div class="ranking-grid">${sources.map(source=>{const rows=(source.rows||[]).slice(0,3);return `<article class="ranking-card ${source.status==='confirmed'?'confirmed':'pending'}"><div class="ranking-card-head"><span>${esc(source.perspective||'評価軸')}</span><a href="${esc(source.url)}" target="_blank" rel="noreferrer">ランキング ↗</a></div><h3>${esc(source.label)}</h3>${source.status==='confirmed'?`<ol>${rows.map(row=>`<li><b>${String(row.rank).padStart(2,'0')}</b><div><strong>${esc(row.model)}</strong><small>${esc(row.provider||'')}</small></div></li>`).join('')}</ol><footer><span>${esc(source.as_of||'基準日未記録')}</span><a href="${esc(source.url)}" target="_blank" rel="noreferrer">詳細を見る ↗</a></footer>`:`<div class="ranking-pending"><b>次回の週次調査で確認</b></div>`}</article>`;}).join('')}</div></section>`;
}

function renderWeeklyReport(signals,allSignals,brief){
  const active=state.digestCategory;
  const official=signals.map(signal=>({kind:'official',id:signal.id,theme:weeklyLensForSignal(signal),title:signal.title,summary:signal.summary,business_note:signal.why_it_matters,source:signal.source?.publisher||'一次情報',source_url:signal.source?.url,published_at:signal.published_at,evidence:'primary',entity:signal.entity?.name||signal.entity_id}));
  const curated=(brief.weekly_digest||[]).map(item=>({...item,kind:'curated'}));
  const allItems=[...official,...curated].sort((a,b)=>String(b.published_at).localeCompare(String(a.published_at)));
  const visible=active==='all'?allItems:allItems.filter(item=>item.theme===active);
  const lenses=weeklyLenses.map((lens,index)=>({...lens,index,items:visible.filter(item=>item.theme===lens.id)})).filter(lens=>lens.items.length);
  const sourceCount=new Set(allItems.map(item=>item.source).filter(Boolean)).size;
  const itemCard=item=>`<article class="weekly-story ${item.kind}"><a href="${esc(item.source_url||'#')}" target="_blank" rel="noreferrer"><div class="weekly-item-meta"><span class="evidence">原典</span><small>${esc(item.source)} · ${esc(item.published_at)}</small></div><h4>${esc(item.title)}</h4><p>${esc(item.summary)}</p><div class="weekly-item-meaning"><b>事業・業務への意味</b><span>${esc(item.business_note)}</span></div><footer>原典を開く <b>↗</b></footer></a></article>`;
  return `<section class="weekly-report"><header class="weekly-report-head"><div><span>WEEKLY INTELLIGENCE REPORT</span><h2>今週のAI市場・企業動向</h2><p>今週の主題：${esc(brief.headline||'AI市場の主要な動き')}</p><small>重要な公開情報を、発信元と原典リンク付きで整理しています。</small></div><dl><div><dt>掲載記事</dt><dd>${allItems.length}<small>件</small></dd></div><div><dt>情報発信元</dt><dd>${sourceCount}<small>組織</small></dd></div></dl></header><nav class="weekly-report-filters" aria-label="週報のテーマ"><button class="${active==='all'?'active':''}" data-digest-filter="all">すべて <b>${allItems.length}</b></button>${weeklyLenses.map(lens=>{const count=allItems.filter(item=>item.theme===lens.id).length;return `<button class="${active===lens.id?'active':''}" data-digest-filter="${esc(lens.id)}">${esc(lens.label)} <b>${count}</b></button>`;}).join('')}</nav><div class="weekly-report-layout"><main>${lenses.length?lenses.map(lens=>`<section class="weekly-topic"><header><i>${String(lens.index+1).padStart(2,'0')}</i><div><h3>${esc(lens.label)}</h3><p>${esc(weeklyLensIntro(lens.id))}</p></div><b>${lens.items.length}<small>件</small></b></header><div class="weekly-topic-items">${lens.items.map(itemCard).join('')}</div></section>`).join(''):`<div class="weekly-report-empty"><b>このテーマの掲載記事はありません。</b><p>フィルターを切り替えて確認してください。</p></div>`}</main><aside class="weekly-report-aside"><section><span>THIS WEEK'S TAKEAWAYS</span><h3>今週の読み解き</h3><p>掲載した公開情報を踏まえた編集上の整理です。</p>${(brief.discussion_points||[]).slice(0,3).map((point,index)=>`<div><i>${String(index+1).padStart(2,'0')}</i><p>${esc(point)}</p></div>`).join('')||'<p>今週の更新をもとに、ここへ読み解きを追加します。</p>'}</section><section class="weekly-report-index"><span>CONTENTS</span><h3>今週のテーマ</h3>${weeklyLenses.map((lens,index)=>{const count=allItems.filter(item=>item.theme===lens.id).length;return `<button class="${active===lens.id?'active':''}" data-digest-filter="${esc(lens.id)}"><i>${String(index+1).padStart(2,'0')}</i><span>${esc(lens.label)}</span><b>${count}</b></button>`;}).join('')}</section></aside></div></section>`;
}

function weeklyLensIntro(id){
  return ({models:'モデル、算力、クラウドなど、AIを動かす技術・提供基盤の更新です。',products:'製品、AIエージェント、SaaSへの実装など、提供形態の更新です。',industry:'事業会社での導入、内製化、顧客・業界向け活用の動きです。',research:'論文、評価、安全性、研究開発に関する確認済みの動きです。',practice:'構想から実装、現場定着までの支援方法と提供体制の更新です。',perspectives:'提携、組織、ガバナンスなど、競争環境と事業条件の動きです。'}[id]||'今週確認した動きです。');
}
...
function themeFamily(category=''){
  const value=String(category);
  if(/半導体|算力|GPU|インフラ/.test(value))return '能力供給';
  if(/モデル|研究/.test(value))return '能力供給';
  if(/エージェント|アプリ|SaaS/.test(value))return '業務実装';
  if(/導入|活用|オファリング|コンサル/.test(value))return '事業化';
  if(/ガバナンス|安全|統制|セキュリティ/.test(value))return '統制';
...
function renderStructureMap(window){
  ...
  const summary=groups.map(group=>({ ...group, companies:new Set(group.signals.map(item=>item.entity_id)).size, latest:group.signals.slice().sort((a,b)=>String(b.published_at).localeCompare(String(a.published_at)))[0] }));
  return `<section class="structure-map"><header><div><span>MARKET STRUCTURE MAP</span><h2>AI市場の構造</h2><p>事実としての因果ではなく、確認済み更新を「どの役割の動きか」でつないだ分析上の見取り図です。</p></div><small>矢印 = 分析上の接続</small></header><div class="structure-flow">${summary.map((group,index)=>`<article class="structure-node ${group.signals.length?'active':''}"><span>${String(index+1).padStart(2,'0')}</span><h3>${esc(group.label)}</h3><b>${group.signals.length}<small>件</small></b><p>${group.latest?esc(shortLabel(group.latest.title,42)):'選択期間の確認なし'}</p><footer>${group.companies}社が関与</footer></article>${index<summary.length-1?'<i class="structure-arrow" aria-hidden="true">→</i>':''}`).join('')}</div><footer class="structure-note">能力供給が業務実装を支え、事業化が広がるほど統制・エコシステムの論点が強まる、という会議用の読み方です。個別の根拠は各テーマ・企業の詳細から確認します。</footer></section>`;
}
...
function renderThemeSummary(){
  ...
  return `<div class="market-intelligence"><section class="intel-hero"><div><span>MARKET INTELLIGENCE / ${esc(window.definition.label)}</span><h2>AI市場の構造変化を読む</h2><p>技術・実装・事業化・統制のどこに動きが集中しているかを、確認済みの原典から一枚で把握します。</p></div><dl><div><dt>確認済み更新</dt><dd>${window.current.length}<small>件</small></dd></div><div><dt>関与企業</dt><dd>${companies}<small>社</small></dd></div><div><dt>基準日</dt><dd>${esc(window.reference)}</dd></div></dl></section>${renderPeriodScopePanel()}<div class="intel-main-grid">${renderMarketRadar(window)}${renderInsightRail()}</div>${renderStructureMap(window)}${renderActivityMatrix(window)}${renderPlayerUpdates(window)}</div>`;
}
