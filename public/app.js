...
function renderWeeklyPulse(signals,allSignals){
  ...
  return `<section class="pulse-board"><header><div><span>WEEKLY AI MARKET PULSE</span><h2>今週のAI市場の動き</h2><p>週次調査の6つの切り口で、公式情報として確認できた発表・導入・提携を配置します。</p></div></header><nav class="pulse-filters" aria-label="週次の観測切り口"><button class="${active==='all'?'active':''}" data-digest-filter="all">すべて <b>${allSignals.length}</b></button>${weeklyLenses.map(lens=>{const count=allSignals.filter(signal=>matchesWeeklyLens(signal,lens.id)).length;return `<button class="${active===lens.id?'active':''}" data-digest-filter="${esc(lens.id)}">${esc(lens.label)} <b>${count}</b></button>`;}).join('')}</nav><div class="pulse-body"><div class="pulse-lanes">${weeklyLenses.map((lens,index)=>{const items=allSignals.filter(signal=>matchesWeeklyLens(signal,lens.id));const visible=active==='all'?items:items.filter(signal=>matchesWeeklyLens(signal,active));const item=visible[0];return `<button class="pulse-lane ${item?'has-update':''} ${active===lens.id?'selected':''}" data-digest-filter="${esc(lens.id)}"><i>${String(index+1).padStart(2,'0')}</i><div><strong>${esc(lens.label)}</strong><small>${esc(lens.id==='models'?'モデル、計算基盤、提供条件':lens.id==='products'?'製品、エージェント、SaaSへの組み込み':lens.id==='industry'?'導入、内製化、顧客・業界活用':lens.id==='research'?'論文、評価、安全性、研究開発':lens.id==='practice'?'実装、運用、業務設計、定着': '競争、提携、投資、規制、組織')}</small></div>${item?`<span><b>●</b>${esc(item.entity?.name||item.entity_id)}<em>${esc(shortLabel(item.title,28))}</em></span>`:'<span class="pulse-none">今週の公式確認なし</span>'}</button>`;}).join('')}</div><aside class="pulse-evidence"><header><span>OFFICIAL UPDATES</span><h3>今週の確認情報</h3><p>点を押すと原典を開きます。</p></header>${ranked.length?ranked.map((signal,index)=>`<button data-signal="${esc(signal.id)}"><i>${String(index+1).padStart(2,'0')}</i><div><b>${esc(signal.entity?.name||signal.entity_id)}</b><strong>${esc(shortLabel(signal.title,54))}</strong><small>${esc(signal.source?.publisher||'一次情報')} · ${esc(signal.published_at)}</small></div></button>`).join(''):'<div class="pulse-empty">この切り口の公式確認情報はありません。</div>'}</aside></div><footer><span class="pulse-dot">●</span> 今週、公式情報で確認した更新　<span class="pulse-line">—</span> 今週は公式確認なし（活動がないことを意味しません）</footer></section>`;
}
