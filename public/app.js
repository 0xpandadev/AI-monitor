...
function renderDigest(){
  ...
  const scopeLabel=b.scope?.label;
  ...
  return `<section class="canvas-hero"><div class="canvas-hero-copy"><span>AI OPPORTUNITY MONITOR · ${esc(period)}</span><h2>${esc(scopeLabel?`${scopeLabel}で確認した主な動き`:'今週のAI市場で確認した主な動き')}</h2><p>${esc(scopeLabel?'対象企業の公式発表を確認し、掲載対象を限定した試行レポートです。':'AI市場全般の技術・製品・活用トレンドと、監視企業の公式更新を確認しています。')}</p><div class="canvas-brief-messages count-${Math.max(1,keyMessages.length)}">${keyMessages.length?keyMessages.map((signal,index)=>`<button data-signal="${esc(signal.id)}"><i>${String(index+1).padStart(2,'0')}</i><div><b>${esc(shortLabel(signal.title,52))}</b><small>${esc(signal.entity?.name||signal.entity_id)} · ${esc(signal.category)} · ${esc(signal.published_at)}</small></div></button>`).join(''):`<div><b>${esc(b.headline)}</b><small>今週の公式確認情報を取り込むと、ここに主要メッセージを表示します。</small></div>`}</div></div><aside class="canvas-coverage"><span>RESEARCH COVERAGE</span><dl><div><dt>監視企業</dt><dd>${d.metrics.watched}</dd></div><div><dt>${esc(scopeLabel?'今回の掲載':'確認済み更新')}</dt><dd>${feed.length}</dd></div><div><dt>基礎情報あり</dt><dd>${d.metrics.profiles_started}<small> / ${d.metrics.watched}</small></dd></div></dl><p>${esc(scopeLabel?`${scopeLabel}。全社一巡の完了状況ではありません。`:d.profile_coverage.note||'3〜5年ベースラインを整備中です。')}</p><button data-jump="settings">調査範囲・判定ルール →</button></aside></section>
...
function renderWeeklyReport(signals,allSignals,brief){
  ...
  const sorted=signals.slice().sort((a,b)=>{const weight={critical:3,high:2,medium:1,low:0};return (weight[b.importance]||0)-(weight[a.importance]||0)||String(b.published_at).localeCompare(String(a.published_at));});
  const lenses=weeklyLenses.map((lens,index)=>({...lens,index,signals:sorted.filter(signal=>matchesWeeklyLens(signal,lens.id))})).filter(lens=>lens.signals.length);
  const sourceCount=new Set(sorted.map(signal=>signal.source?.publisher).filter(Boolean)).size;
  return `<section class="weekly-report"><header class="weekly-report-head"><div><span>WEEKLY INTELLIGENCE REPORT</span><h2>今週のAI市場・企業動向</h2><p>公式根拠を確認した${allSignals.length}件を、技術、製品、企業活用、実装、競争環境の観点から編集しています。</p></div><dl><div><dt>確認済み</dt><dd>${allSignals.length}<small>件</small></dd></div><div><dt>情報発信元</dt><dd>${sourceCount}<small>組織</small></dd></div></dl></header><nav class="weekly-report-filters" aria-label="週報のテーマ"><button class="${active==='all'?'active':''}" data-digest-filter="all">すべて <b>${allSignals.length}</b></button>${weeklyLenses.map(lens=>{const count=allSignals.filter(signal=>matchesWeeklyLens(signal,lens.id)).length;return `<button class="${active===lens.id?'active':''}" data-digest-filter="${esc(lens.id)}">${esc(lens.label)} <b>${count}</b></button>`;}).join('')}</nav><div class="weekly-report-layout"><main>${lenses.length?lenses.map(lens=>`<section class="weekly-topic"><header><i>${String(lens.index+1).padStart(2,'0')}</i><div><h3>${esc(lens.label)}</h3><p>${esc(weeklyLensIntro(lens.id))}</p></div><b>${lens.signals.length}<small>件</small></b></header><div class="weekly-topic-items">${lens.signals.map(signal=>`<article><button data-signal="${esc(signal.id)}"><div class="weekly-item-meta"><span>${esc(signal.entity?.name||signal.entity_id)}</span><small>${esc(signal.source?.publisher||'一次情報')} · ${esc(signal.published_at)}</small></div><h4>${esc(signal.title)}</h4><p>${esc(signal.summary)}</p><div class="weekly-item-meaning"><b>事業・業務への意味</b><span>${esc(signal.why_it_matters)}</span></div></button></article>`).join('')}</div></section>`).join(''):`<div class="weekly-report-empty"><b>このテーマの確認済み情報はありません。</b><p>活動がないことを意味するものではありません。一次情報を確認できたものだけを表示します。</p></div>`}</main><aside class="weekly-report-aside"><section><span>THIS WEEK'S TAKEAWAYS</span><h3>今週の読み解き</h3><p>下記は、確認済みの根拠を踏まえた編集上の整理です。</p>${(brief.discussion_points||[]).slice(0,3).map((point,index)=>`<div><i>${String(index+1).padStart(2,'0')}</i><p>${esc(point)}</p></div>`).join('')||'<p>確認済みの更新をもとに、ここへ週次の読み解きを追加します。</p>'}</section><section class="weekly-report-index"><span>CONTENTS</span><h3>今週のテーマ</h3>${weeklyLenses.map((lens,index)=>{const count=allSignals.filter(signal=>matchesWeeklyLens(signal,lens.id)).length;return `<button class="${active===lens.id?'active':''}" data-digest-filter="${esc(lens.id)}"><i>${String(index+1).padStart(2,'0')}</i><span>${esc(lens.label)}</span><b>${count}</b></button>`;}).join('')}</section></aside></div></section>`;
}
