...
function renderDigest(){
  ...
  ${renderWeeklyPulse(activeFeed,feed)}
...
function renderRankingPulse(rankings){
  ...
  return `<section class="ranking-board"><header><div><span>RANKING LENSES</span><h2>モデル・利用動向ランキング</h2><p>同じ「順位」でも測っているものは異なります。性能、利用、人間選好、実装能力を混ぜずに確認します。</p></div><aside><b>${sources.filter(item=>item.status==='confirmed').length}</b><small> / ${sources.length} 指標を更新</small></aside></header><div class="ranking-grid">${sources.map(source=>{const rows=(source.rows||[]).slice(0,3);return `<article class="ranking-card ${source.status==='confirmed'?'confirmed':'pending'}"><div class="ranking-card-head"><span>${esc(source.perspective||'評価軸')}</span><a href="${esc(source.url)}" target="_blank" rel="noreferrer">原典を開く ↗</a></div><h3>${esc(source.label)}</h3><p>${esc(source.metric)}</p>${source.status==='confirmed'?`<ol>${rows.map(row=>`<li><b>${String(row.rank).padStart(2,'0')}</b><div><strong>${esc(row.model)}</strong><small>${esc(row.provider||'')} · ${esc(row.metric)}${row.change?` · ${esc(row.change)}`:''}</small></div></li>`).join('')}</ol><footer><span>基準日 ${esc(source.as_of||'未記録')}</span><a href="${esc(source.methodology_url||source.url)}" target="_blank" rel="noreferrer">測り方</a></footer>`:`<div class="ranking-pending"><b>次回の週次調査で確認</b><small>更新日と原典を確認してから表示します。</small></div>`}<details><summary>この順位が示さないこと</summary><p>${esc(source.limitation||'測定範囲外の評価はしません。')}</p></details></article>`;}).join('')}</div><footer class="ranking-note">順位や点数を横断して合算・総合順位化しません。各カードは、その情報源が測定した範囲だけを示します。</footer></section>`;
}

function renderWeeklyPulse(signals,allSignals){
  const active=state.digestCategory;
  const ranked=signals.slice().sort((a,b)=>String(b.published_at).localeCompare(String(a.published_at))).slice(0,4);
  return `<section class="pulse-board"><header><div><span>WEEKLY AI MARKET PULSE</span><h2>今週のAI市場の動き</h2><p>週次調査の6つの切り口で、公式情報として確認できた発表・導入・提携を配置します。</p></div></header><nav class="pulse-filters" aria-label="週次の観測切り口"><button class="${active==='all'?'active':''}" data-digest-filter="all">すべて <b>${allSignals.length}</b></button>${weeklyLenses.map(lens=>{const count=allSignals.filter(signal=>matchesWeeklyLens(signal,lens.id)).length;return `<button class="${active===lens.id?'active':''}" data-digest-filter="${esc(lens.id)}">${esc(lens.label)} <b>${count}</b></button>`;}).join('')}</nav><div class="pulse-body"><div class="pulse-lanes">${weeklyLenses.map((lens,index)=>{const count=allSignals.filter(signal=>matchesWeeklyLens(signal,lens.id)).length;return `<button class="pulse-lane ${count?'has-update':''} ${active===lens.id?'selected':''}" data-digest-filter="${esc(lens.id)}"><i>${String(index+1).padStart(2,'0')}</i><div><strong>${esc(lens.label)}</strong><small>${esc(lens.id==='models'?'モデル、計算基盤、提供条件':lens.id==='products'?'製品、エージェント、SaaSへの組み込み':lens.id==='industry'?'導入、内製化、顧客・業界活用':lens.id==='research'?'論文、評価、安全性、研究開発':lens.id==='practice'?'実装、運用、業務設計、定着': '競争、提携、投資、規制、組織')}</small></div><b>${count?`● ${count}`:'—'}</b></button>`;}).join('')}</div><aside class="pulse-evidence"><header><span>OFFICIAL UPDATES</span><h3>今週の確認情報</h3><p>タイトルを押すと原典を開きます。</p></header>${ranked.length?ranked.map((signal,index)=>`<button data-signal="${esc(signal.id)}"><i>${String(index+1).padStart(2,'0')}</i><div><b>${esc(signal.entity?.name||signal.entity_id)}</b><strong>${esc(signal.title)}</strong><small>${esc(signal.source?.publisher||'一次情報')} · ${esc(signal.published_at)}</small></div></button>`).join(''):'<div class="pulse-empty">この切り口の公式確認情報はありません。</div>'}</aside></div><footer><span class="pulse-dot">●</span> 今週、公式情報で確認した更新　<span class="pulse-line">—</span> 今週は公式確認なし（活動がないことを意味しません）</footer></section>`;
}
