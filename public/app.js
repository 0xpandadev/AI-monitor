...
function renderDigest(){
  ...
  return `<section class="canvas-hero"><div class="canvas-hero-copy"><span>AI OPPORTUNITY MONITOR · ${esc(period)}</span><h2>今週のAI市場で確認した主な動き</h2><p>AIhotの選定記事と、監視企業・研究機関の原典を編集して、技術、製品、企業活用、研究、事業環境をまとめます。</p><div class="canvas-brief-messages count-${Math.max(1,keyMessages.length)}">${keyMessages.length?keyMessages.map((signal,index)=>`<button data-signal="${esc(signal.id)}"><i>${String(index+1).padStart(2,'0')}</i><div><b>${esc(shortLabel(signal.title,52))}</b><small>${esc(signal.entity?.name||signal.entity_id)} · ${esc(signal.category)} · ${esc(signal.published_at)}</small></div></button>`).join(''):`<div><b>${esc(b.headline)}</b><small>今週の更新を取り込むと、ここに主要メッセージを表示します。</small></div>`}</div></div><aside class="canvas-coverage"><span>RESEARCH COVERAGE</span><dl><div><dt>監視企業</dt><dd>${d.metrics.watched}</dd></div><div><dt>公式確認</dt><dd>${feed.length}</dd></div><div><dt>AIhot選定</dt><dd>${(b.weekly_digest||[]).length}</dd></div></dl><p>${esc(d.profile_coverage.note||'3〜5年ベースラインを整備中です。')}</p><button data-jump="settings">調査範囲・判定ルール →</button></aside></section>
...
function renderWeeklyReport(signals,allSignals,brief){
  ...
  const itemCard=item=>`<article class="weekly-story ${item.kind}"><a href="${esc(item.source_url||'#')}" target="_blank" rel="noreferrer"><div class="weekly-item-meta"><span class="evidence ${esc(item.evidence||'curated')}">${item.evidence==='primary'?'公式確認':'AIhot選定'}</span><small>${esc(item.source)} · ${esc(item.published_at)}</small></div><h4>${esc(item.title)}</h4><p>${esc(item.summary)}</p><div class="weekly-item-meaning"><b>事業・業務への意味</b><span>${esc(item.business_note)}</span></div><footer>原典を開く <b>↗</b></footer></a></article>`;
  return `<section class="weekly-report"><header class="weekly-report-head"><div><span>WEEKLY INTELLIGENCE REPORT</span><h2>今週のAI市場・企業動向</h2><p>今週の主題：${esc(brief.headline||'AI市場の主要な動き')}</p><small>公式確認 ${official.length}件と、AIhotが選定した重要記事 ${curated.length}件を、原典リンク付きで編集しています。</small></div><dl><div><dt>掲載記事</dt><dd>${allItems.length}<small>件</small></dd></div><div><dt>情報発信元</dt><dd>${sourceCount}<small>組織</small></dd></div></dl></header><nav class="weekly-report-filters" aria-label="週報のテーマ"><button class="${active==='all'?'active':''}" data-digest-filter="all">すべて <b>${allItems.length}</b></button>${weeklyLenses.map(lens=>{const count=allItems.filter(item=>item.theme===lens.id).length;return `<button class="${active===lens.id?'active':''}" data-digest-filter="${esc(lens.id)}">${esc(lens.label)} <b>${count}</b></button>`;}).join('')}</nav><div class="weekly-report-layout"><main>${lenses.length?lenses.map(lens=>`<section class="weekly-topic"><header><i>${String(lens.index+1).padStart(2,'0')}</i><div><h3>${esc(lens.label)}</h3><p>${esc(weeklyLensIntro(lens.id))}</p></div><b>${lens.items.length}<small>件</small></b></header><div class="weekly-topic-items">${lens.items.map(itemCard).join('')}</div></section>`).join(''):`<div class="weekly-report-empty"><b>このテーマの掲載記事はありません。</b><p>フィルターを切り替えて確認してください。</p></div>`}</main><aside class="weekly-report-aside"><section><span>THIS WEEK'S TAKEAWAYS</span><h3>今週の読み解き</h3><p>確認済みの根拠と選定記事を踏まえた編集上の整理です。</p>${(brief.discussion_points||[]).slice(0,3).map((point,index)=>`<div><i>${String(index+1).padStart(2,'0')}</i><p>${esc(point)}</p></div>`).join('')||'<p>今週の更新をもとに、ここへ読み解きを追加します。</p>'}</section><section class="weekly-report-index"><span>CONTENTS</span><h3>今週のテーマ</h3>${weeklyLenses.map((lens,index)=>{const count=allItems.filter(item=>item.theme===lens.id).length;return `<button class="${active===lens.id?'active':''}" data-digest-filter="${esc(lens.id)}"><i>${String(index+1).padStart(2,'0')}</i><span>${esc(lens.label)}</span><b>${count}</b></button>`;}).join('')}</section></aside></div></section>`;
}
