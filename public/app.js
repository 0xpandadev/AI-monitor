...
function renderDigest(){
  ...
  return `<section class="canvas-hero"><div class="canvas-hero-copy"><span>AI OPPORTUNITY MONITOR · ${esc(period)}</span><h2>今週のAI市場で確認した主な動き</h2><p>技術、製品、企業活用、研究、事業環境の重要な公開情報を、原典リンク付きで整理します。</p><div class="canvas-brief-messages count-${Math.max(1,keyMessages.length)}">${keyMessages.length?keyMessages.map((signal,index)=>`<button data-signal="${esc(signal.id)}"><i>${String(index+1).padStart(2,'0')}</i><div><b>${esc(shortLabel(signal.title,52))}</b><small>${esc(signal.entity?.name||signal.entity_id)} · ${esc(signal.category)} · ${esc(signal.published_at)}</small></div></button>`).join(''):`<div><b>${esc(b.headline)}</b><small>今週の更新を取り込むと、ここに主要メッセージを表示します。</small></div>`}</div></div><aside class="canvas-coverage"><span>RESEARCH COVERAGE</span><dl><div><dt>監視企業</dt><dd>${d.metrics.watched}</dd></div><div><dt>掲載記事</dt><dd>${feed.length+(b.weekly_digest||[]).length}</dd></div><div><dt>基礎情報あり</dt><dd>${d.metrics.profiles_started}</dd></div></dl><p>${esc(d.profile_coverage.note||'3〜5年ベースラインを整備中です。')}</p><button data-jump="settings">調査範囲・判定ルール →</button></aside></section>
