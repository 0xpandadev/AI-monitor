...
function renderDigest(){
  ...
  const activeFeed=state.digestCategory==='all'?feed:feed.filter(signal=>signal.category===state.digestCategory);
  return `<section class="canvas-hero"><div class="canvas-hero-copy"><span>AI OPPORTUNITY MONITOR · ${esc(period)}</span><h2>${esc(b.headline)}</h2><p>${esc(b.summary)}</p><div class="canvas-prompt"><b>今回、会議で決めること</b>${b.discussion_points?.length?`<span>${esc(b.discussion_points[0])}</span>`:'<span>重要な示唆を根拠とともに確認します。</span>'}</div></div><aside class="canvas-coverage"><span>RESEARCH COVERAGE</span><dl><div><dt>監視企業</dt><dd>${d.metrics.watched}</dd></div><div><dt>確認済み更新</dt><dd>${d.metrics.this_period}</dd></div><div><dt>基礎情報あり</dt><dd>${d.metrics.profiles_started}<small> / ${d.metrics.watched}</small></dd></div></dl><p>${esc(d.profile_coverage.note||'3〜5年ベースラインを整備中です。')}</p><button data-jump="settings">調査範囲・判定ルール →</button></aside></section>
...
const monitorThemes=['基盤モデル','AIエージェント','AIオファリング','内製開発','SaaS再定義','ガバナンス','提携・M&A','研究'];
function renderCanvasSignals(signals,allSignals){
  ...
  return `<section class="canvas-signals"><div class="canvas-signals-main"><header><div><span>THIS PERIOD · CONFIRMED SIGNALS</span><h2>今週の重要シグナル</h2><p>一次情報で確認できた情報だけを表示します。ニュースの件数ではなく、会議で読むべき動きを選びます。</p></div><b>${signals.length}<small>件</small></b></header>${lead?`<button class="canvas-lead" data-signal="${esc(lead.id)}"><div><span>${esc(lead.entity?.name||lead.entity_id)}</span><em class="importance ${esc(lead.importance)}">${esc(lead.importance)}</em></div><h3>${esc(lead.title)}</h3><p>${esc(lead.summary)}</p><footer><span>${esc(lead.category)}</span><small>${esc(lead.source?.publisher||'一次情報')} · ${esc(lead.published_at)}</small></footer></button>`:'<div class="empty-inline"><b>このテーマの確認済み情報はありません</b><p>選択期間に一次情報が確認されると、ここに表示されます。</p></div>'}<div class="canvas-supporting">${supporting.map(signal=>`<button data-signal="${esc(signal.id)}"><span>${esc(signal.entity?.name||signal.entity_id)}</span><b>${esc(shortLabel(signal.title,54))}</b><small>${esc(signal.source?.publisher||'一次情報')} · ${esc(signal.published_at)}</small></button>`).join('')}</div></div><aside class="canvas-filter"><header><span>MONITORING THEMES</span><h3>監視テーマ</h3><p>今週の件数にかかわらず、定点観測するテーマを常に表示します。</p></header><button class="${active==='all'?'active':''}" data-digest-filter="all"><span>すべてのテーマ</span><b>${allSignals.length}</b></button>${monitorThemes.map(category=>{const count=allSignals.filter(signal=>signal.category===category).length;return `<button class="${active===category?'active':''}" data-digest-filter="${esc(category)}"><span>${esc(category)}</span><b>${count}</b></button>`;}).join('')}</aside></section>`;
}
