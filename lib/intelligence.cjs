...
function featureRecord(value,profileEvidence=[]){
  const directEvidence=value&&typeof value==='object'&&Array.isArray(value.evidence_signal_ids)?value.evidence_signal_ids:[];
  const name=value&&typeof value==='object'?String(value.name||value.label||'').trim():String(value||'').trim();
  return {
    name,
    evidence_ids:directEvidence,
    profile_evidence_ids:profileEvidence,
    evidence_scope:directEvidence.length?'item':profileEvidence.length?'profile':'unverified'
  };
}
...
function buildKnowledgeGraph({entities,signals,ontology}){
  ...
  const addEdge=edge=>{
    ...
    if(existing){
      existing.evidence_ids=unique([...existing.evidence_ids,...edge.evidence_ids]);
      existing.profile_evidence_ids=unique([...(existing.profile_evidence_ids||[]),...(edge.profile_evidence_ids||[])]);
      existing.evidence_count=existing.evidence_ids.length;
      existing.evidence_scope=existing.evidence_count?'item':existing.profile_evidence_ids.length?'profile':'unverified';
      return;
    }
    const evidenceIds=unique(edge.evidence_ids);const profileEvidenceIds=unique(edge.profile_evidence_ids||[]);
    edges.set(edge.id,{...edge,evidence_ids:evidenceIds,profile_evidence_ids:profileEvidenceIds,evidence_count:evidenceIds.length,evidence_scope:evidenceIds.length?'item':profileEvidenceIds.length?'profile':'unverified'});
  };
  ...
  for(const entity of activeEntities){
    ...
    for(const signal of relatedSignals){
      ...
      addEdge({id:`changed-in:${entity.id}:${topicId}`,from:`company:${entity.id}`,to:topicId,relation_type:'changed-in',label:'変化を確認',evidence_ids:[signal.id],profile_evidence_ids:[],evidence_scope:'item'});
    }
    ...
    for(const raw of entity.profile?.offerings||[]){
      ...
      addEdge({id:`offers:${entity.id}:${offeringId}`,from:`company:${entity.id}`,to:offeringId,relation_type:'offers',label:'提供',evidence_ids:item.evidence_ids,profile_evidence_ids:item.profile_evidence_ids,evidence_scope:item.evidence_scope});
    }
    for(const raw of entity.profile?.partnerships||[]){
      ...
      addEdge({id:`partners-with:${entity.id}:${partnerId}`,from:`company:${entity.id}`,to:partnerId,relation_type:'partners-with',label:'提携',evidence_ids:item.evidence_ids,profile_evidence_ids:item.profile_evidence_ids,evidence_scope:item.evidence_scope});
    }
  }
  const changedByCompany=new Map();
  for(const edge of edges.values())if(edge.relation_type==='changed-in'){
    const list=changedByCompany.get(edge.from)||[];list.push(edge);changedByCompany.set(edge.from,list);
  }
  const topicPairs=new Map();
  for(const [companyNodeId,companyEdges] of changedByCompany){
    const uniqueTopics=[...new Map(companyEdges.map(edge=>[edge.to,edge])).values()].sort((a,b)=>a.to.localeCompare(b.to));
    for(let left=0;left<uniqueTopics.length;left++)for(let right=left+1;right<uniqueTopics.length;right++){
      const leftEdge=uniqueTopics[left],rightEdge=uniqueTopics[right],id=`co-occurs-with:${leftEdge.to}:${rightEdge.to}`;
      const pair=topicPairs.get(id)||{id,from:leftEdge.to,to:rightEdge.to,relation_type:'co-occurs-with',label:'同一企業で共起',evidence_ids:[],profile_evidence_ids:[],supporting_company_evidence:[]};
      const support={entity_id:companyNodeId.replace(/^company:/,''),left_evidence_ids:[...leftEdge.evidence_ids],right_evidence_ids:[...rightEdge.evidence_ids]};
      support.evidence_ids=unique([...support.left_evidence_ids,...support.right_evidence_ids]);
      pair.evidence_ids=unique([...pair.evidence_ids,...support.evidence_ids]);pair.supporting_company_evidence.push(support);topicPairs.set(id,pair);
    }
  }
  for(const pair of topicPairs.values())addEdge(pair);
  ...
  const relationEvidenceCounts=[...edges.values()].reduce((result,edge)=>{
    const bucket=result[edge.relation_type]||(result[edge.relation_type]={item:0,profile:0,unverified:0});
    bucket[edge.evidence_scope]=(bucket[edge.evidence_scope]||0)+1;
    return result;
  },{});
  return {
    ...
    nodes:[...nodes.values()],
    edges:[...edges.values()],
    summary:{nodes:nodes.size,edges:edges.size,companies:activeEntities.length,watched_companies:entities.length,relation_counts:relationCounts,relation_evidence_counts:relationEvidenceCounts},
    caveat:'関係の登録数と、個別根拠がある数は別です。プロフィール根拠のみの項目は、個別の提供・提携を直接証明するものではありません。'
  };
}
...
function buildTrends({signals}){
  ...
  const latestTime=Math.max(0,...confirmed.map(signal=>Date.parse(signal.published_at)||0));
  const day=24*60*60*1000,latestDate=latestTime?new Date(latestTime):null;
  const inWindow=(signal,days)=>latestTime&&Date.parse(signal.published_at)>=latestTime-days*day;
  const beforeWindow=(signal,newerThan,olderThan)=>latestTime&&(Date.parse(signal.published_at)>=latestTime-olderThan*day&&Date.parse(signal.published_at)<latestTime-newerThan*day);
  const momentumThemes=[...categoryGroups].map(([category,items])=>{
    const recent90=items.filter(signal=>inWindow(signal,90));
    const previous90=items.filter(signal=>beforeWindow(signal,90,180));
    const recent365=items.filter(signal=>inWindow(signal,365));
    let momentum='蓄積のみ';
    if(recent90.length&&previous90.length&&recent90.length>previous90.length)momentum='加速';
    else if(recent90.length&&previous90.length)momentum='継続';
    else if(recent90.length&&!previous90.length)momentum='新規・再浮上';
    const latest=items.slice().sort((a,b)=>String(b.published_at).localeCompare(String(a.published_at)))[0];
    return {
      category,
      momentum,
      recent_90_days:recent90.length,
      previous_90_days:previous90.length,
      recent_365_days:recent365.length,
      total_confirmed:items.length,
      companies:unique(items.map(item=>item.entity_id)).length,
      sources:unique(items.map(item=>item.source?.url)).length,
      latest_at:latest?.published_at||null,
      latest_title:latest?.title||'',
      latest_entity:latest?.entity_id||''
    };
  }).sort((a,b)=>b.recent_90_days-a.recent_90_days||b.total_confirmed-a.total_confirmed||a.category.localeCompare(b.category,'ja'));
  return {
    monthly,
    categories,
    momentum_themes:momentumThemes,
    reference_date:latestDate?latestDate.toISOString().slice(0,10):null,
    temporal_points:monthly.length,
    sufficient_for_line:false,
    minimum_temporal_points:8,
    note:'月別本数の線グラフではなく、確認済み公式更新をテーマ別に束ね、直近90日・過去90日・関与企業数でモメンタムを見ます。件数は登録済み根拠の数であり、市場規模や優劣ではありません。'
  };
}
