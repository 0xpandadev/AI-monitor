...
function buildKnowledgeGraph({entities,signals,ontology}){
  ...
  for(const entity of activeEntities){
    ...
    const profileEvidence=unique(entity.profile?.activity_signal_ids||[]).filter(id=>signalMap.has(id));
...
function buildInsights({entities,signals,ontology,profileCoverage}){
  ...
  for(const [category,items] of categoryGroups){
    ...
    insights.push({
      ...
      counterevidence:`3年ベースライン完了は${profileCoverage.profiles_complete||0} / ${profileCoverage.total_entities||entities.length}社のため、長期トレンドとはまだ断定しません。`,
