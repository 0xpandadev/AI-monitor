...
function dashboard(){
  ...
  return {
    ...
    entities:enriched,
    signals:signals.map(signal=>({...signal,entity:entityMap.get(signal.entity_id)||null})),
...
function importProfileBatch(batch){
  ...
  const coverage={status:profiles.filter(profile=>profile.status==='complete').length===total?'complete':'in_progress',profiles_started:profiles.length,profiles_complete:profiles.filter(profile=>profile.status==='complete').length,total_entities:total,note:'completeは指定した3年窓の一次情報確認が完了したプロフィールのみ'};
