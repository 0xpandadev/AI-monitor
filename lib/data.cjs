...
const PATHS={
  ...
  rankingSources:path.join(ROOT,'config','ranking-sources.json'),
  rankings:path.join(ROOT,'data','rankings.json'),
...
function dashboard(){
  ...
  return {
    ...
    rankings:{...readJson(PATHS.rankings,{schema_version:'1.0',updated_at:null,sources:[]}),catalog:readJson(PATHS.rankingSources,{schema_version:'1.0',sources:[]})},
    intelligence,
...
function validateProfileBatch(batch){
  ...
  for(const [index,profile] of (batch.profiles||[]).entries()){
    ...
    for(const field of ['internal_use','offerings','reusable_assets','partnerships','talent_org'])if(profile[field]!==undefined&&!Array.isArray(profile[field]))errors.push(`${prefix}.${field}は配列です`);
