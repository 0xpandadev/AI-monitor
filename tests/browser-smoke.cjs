...
(async()=>{
  ...
  try{
    ...
    const features=await evaluate(cdp,`(async()=>{
      ...
      document.querySelector('[data-view="relationships"]').click();await new Promise(r=>setTimeout(r,60));const graphNodes=document.querySelectorAll('.graph-node').length;const trendPanel=document.querySelector('.momentum-board')!==null;const themeSummary=document.querySelector('.theme-summary')!==null;const offeringCatalog=document.querySelector('.relationship-catalog')!==null;const partnershipCatalog=document.querySelectorAll('.relationship-catalog').length===2;const graphDetails=document.querySelector('.relationship-explorer')?.open===false;
