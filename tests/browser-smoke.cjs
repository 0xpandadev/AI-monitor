...
(async()=>{
  ...
  try{
    ...
    assert.equal(summary.title,'AI Opportunity Monitor');assert.match(summary.metrics,/監視企業\s*181/);assert.equal(summary.digestCategories>1,true);assert.equal(summary.filteredDigestCards>0,true);assert.equal(summary.startupRows,40);assert.match(summary.startupTitle,/スタートアップ・新興企業/);assert.equal(summary.consultingRows,67);assert.equal(summary.consultingFilters>1,true);assert.equal(summary.big4Rows,4);assert.match(summary.drawer,/現在のAI戦略/);assert.match(summary.drawer,/公式根拠/);
    ...
    const features=await evaluate(cdp,`(async()=>{
      ...
      document.querySelector('[data-view="relationships"]').click();await new Promise(r=>setTimeout(r,60));const graphNodes=document.querySelectorAll('.graph-node').length;const trendPanel=document.querySelector('.category-bars,.monthly-line')!==null;
