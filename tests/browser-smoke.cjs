...
(async()=>{
  ...
  try{
    ...
    const summary=await evaluate(cdp,`(async()=>{
      ...
      click('[data-view="ai-companies"]');await new Promise(r=>setTimeout(r,60));result.aiRows=document.querySelectorAll('.entity-matrix tbody tr').length;result.aiFilters=document.querySelectorAll('[data-company-filter]').length;click('[data-entity="palantir"]');await new Promise(r=>setTimeout(r,60));result.aiDrawer=text('#drawer-content');document.querySelector('#drawer-close').click();
    ...
    assert.equal(summary.title,'AI Opportunity Monitor');assert.match(summary.metrics,/監視企業\s*208/);assert.equal(summary.digestCategories>1,true);assert.equal(summary.filteredDigestCards>0,true);assert.equal(summary.aiRows,37);assert.equal(summary.aiFilters>1,true);assert.match(summary.aiDrawer,/Palantir AIP/);assert.match(summary.aiDrawer,/公式根拠/);assert.equal(summary.startupRows,40);assert.match(summary.startupTitle,/スタートアップ・新興企業/);assert.equal(summary.consultingRows,67);assert.equal(summary.consultingFilters>1,true);assert.equal(summary.big4Rows,4);assert.match(summary.drawer,/現在のAI戦略/);assert.match(summary.drawer,/公式根拠/);
    ...
    const features=await evaluate(cdp,`(async()=>{
      ...
      document.querySelector('[data-view="relationships"]').click();await new Promise(r=>setTimeout(r,60));const graphNodes=document.querySelectorAll('.graph-node').length;const trendPanel=document.querySelector('.momentum-board')!==null;
      ...
      return {industry,ledgerRows,graphNodes,trendPanel,insightCards,insightDrawer,meeting};
    })()`);
    assert.equal(features.industry,true);assert.equal(features.ledgerRows>0,true);assert.equal(features.graphNodes>0,true);assert.equal(features.trendPanel,true);assert.equal(features.insightCards>0,true);assert.equal(features.insightDrawer,true);assert.equal(features.meeting,true);
