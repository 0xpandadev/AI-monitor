...
(async()=>{
  ...
  try{
    ...
    const features=await evaluate(cdp,`(async()=>{
      ...
      document.querySelector('[data-view="relationships"]').click();await new Promise(r=>setTimeout(r,60));const graphNodes=document.querySelectorAll('.graph-node').length;const trendFallback=document.querySelector('.category-bars')!==null;
      ...
      return {industry,ledgerRows,graphNodes,trendFallback,insightCards,insightDrawer,meeting};
    })()`);
    assert.equal(features.industry,true);assert.equal(features.ledgerRows>0,true);assert.equal(features.graphNodes>0,true);assert.equal(features.trendFallback,true);assert.equal(features.insightCards>0,true);assert.equal(features.insightDrawer,true);assert.equal(features.meeting,true);
