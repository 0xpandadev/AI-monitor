  ...
  try{
    ...
    const features=await evaluate(cdp,`(async()=>{
      ...
      document.querySelector('[data-view="ledger"]').click();await new Promise(r=>setTimeout(r,60));const ledgerRows=document.querySelectorAll('.change-ledger tbody tr').length;
       document.querySelector('[data-view="relationships"]').click();await new Promise(r=>setTimeout(r,60));const graphNodes=document.querySelectorAll('.graph-node').length;const themeSummary=document.querySelector('.theme-summary')!==null;const trendPanel=document.querySelector('.momentum-board')===null;const periodButtons=document.querySelectorAll('[data-period-scope]').length;const observatory=document.querySelector('.observatory-hero')!==null;const activityMatrix=document.querySelector('.activity-matrix')!==null;const activityDots=document.querySelectorAll('.activity-dot').length;const readout=document.querySelector('.readout-band')!==null;const consultingUpdates=document.querySelector('.consulting-updates')!==null;document.querySelector('[data-period-scope="quarter"]').click();await new Promise(r=>setTimeout(r,60));const quarterMeta=document.querySelector('.observatory-meta')?.innerText.includes('四半期');const offeringCatalogDefault=document.querySelector('.relationship-catalog')===null;const explorer=document.querySelector('.relationship-explorer');explorer.querySelector('summary').click();await new Promise(r=>setTimeout(r,60));const graphOpenBefore=explorer.open;document.querySelector('[data-graph-filter="offers"]').click();await new Promise(r=>setTimeout(r,60));const graphStaysOpen=document.querySelector('.relationship-explorer')?.open===true;const graphFilterLabels=[...document.querySelectorAll('.graph-filters button')].every(button=>button.innerText.trim().length>4);document.querySelector('[data-relationship-tab="offers"]').click();await new Promise(r=>setTimeout(r,60));const offeringCatalog=document.querySelector('.relationship-catalog')!==null;document.querySelector('[data-relationship-tab="partners"]').click();await new Promise(r=>setTimeout(r,60));const partnershipCatalog=document.querySelector('.relationship-catalog')!==null;const graphDetails=document.querySelector('.relationship-explorer')?.open===true;
      ...
      document.querySelector('#meeting-mode-button').click();await new Promise(r=>setTimeout(r,60));const meeting=!document.querySelector('#meeting-mode').hidden&&document.querySelector('#meeting-content').innerText.includes('THIS WEEK');document.querySelector('#meeting-close').click();
       return {industry,ledgerRows,graphNodes,trendPanel,themeSummary,periodButtons,observatory,activityMatrix,activityDots,readout,consultingUpdates,quarterMeta,graphOpenBefore,graphStaysOpen,graphFilterLabels,offeringCatalogDefault,offeringCatalog,partnershipCatalog,graphDetails,insightCards,insightDrawer,meeting};
    })()`);
     assert.equal(features.industry,true);assert.equal(features.ledgerRows>0,true);assert.equal(features.graphNodes>0,true);assert.equal(features.trendPanel,true);assert.equal(features.themeSummary,true);assert.equal(features.periodButtons,5);assert.equal(features.observatory,true);assert.equal(features.activityMatrix,true);assert.equal(features.activityDots>0,true);assert.equal(features.readout,true);assert.equal(features.consultingUpdates,true);assert.equal(features.quarterMeta,true);assert.equal(features.graphOpenBefore,true);assert.equal(features.graphStaysOpen,true);assert.equal(features.graphFilterLabels,true);assert.equal(features.offeringCatalogDefault,true);assert.equal(features.offeringCatalog,true);assert.equal(features.partnershipCatalog,true);assert.equal(features.graphDetails,true);assert.equal(features.insightCards>0,true);assert.equal(features.insightDrawer,true);assert.equal(features.meeting,true);
