...
const rankingCatalog=require('../config/ranking-sources.json');
...
const manifest={
  ...
  files:{discovery:'discovery.json',verification:'verification.json',rankings:'rankings.json',ontology:'ontology-analysis.json',scenario:'scenario-analysis.json',weekly_update:'weekly-update.json',report:'run-report.md'}
};
...
const rankings={schema_version:'1.0',run_id:runId,generated_at:stamp,sources:rankingCatalog.sources.map(source=>({id:source.id,status:'pending',as_of:null,captured_at:null,rows:[],notes:[]}))};
...
writeJson(path.join(dir,'manifest.json'),manifest);writeJson(path.join(dir,'discovery.json'),discovery);writeJson(path.join(dir,'verification.json'),verification);writeJson(path.join(dir,'rankings.json'),rankings);writeJson(path.join(dir,'ontology-analysis.json'),ontology);writeJson(path.join(dir,'scenario-analysis.json'),scenario);writeJson(path.join(dir,'weekly-update.json'),weeklyUpdate);
